...
function RouteComponent() {
  return (
    <>
      <DashboardTitle title="Dashboard" subtitle="Track your fitness journey and progress" />
...
const DashboardMiniCard = ({ title, content, subContent, icon, progress}: DashboardMiniCardProps) => (
  <article className="bg-white rounded-md p-4 shadow flex flex-col gap-2 lg:w-1/4 lg:justify-evenly dark:bg-gray-900">
    ...
    <section className="flex flex-col gap-1">
      ...
      <p className={`${progress ? "text-green-500 dark:text-green-200" : "text-red-500 dark:text-red-200"} text-xs`}>{subContent}</p>
    </section>
  </article>
)